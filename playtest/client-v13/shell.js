...
const old=document.getElementById('randomBtn'); if(old) old.click();setTimeout(syncShell,0);});const quickStart=document.getElementById('stQuickStart');const localLeave=document.createElement('button');localLeave.id='stLocalLeave';localLeave.type='button';localLeave.className='st-btn';localLeave.textContent='LEAVE';localLeave.onclick=()=>window.__stoneThrowOpenMainMenu?.();quickStart?.after(localLeave);if(quickStart) quickStart.addEventListener('click',()=>{
...
window.__stoneThrowSyncMultiplayerControls?.();
}
