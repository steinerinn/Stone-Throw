...
function visibleScene(){if(document.body.classList.contains('st-group-results-open'))return 'menu';if(!entered)return 'menu';const storyVisible=document.getElementById('stMenuDialog')?.classList.contains('open')||document.getElementById('stStoryPhaseOverlay')?.classList.contains('open');return musicScene({menu:document.body.classList.contains('st-main-menu-mode'),story:!!window.__stoneThrowViewState?.()?.storyMode,storyScene:storyVisible?document.documentElement.dataset.musicStoryScene:null,...publicState});}
