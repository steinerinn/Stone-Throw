...
export function createMusicController({media=()=>new Audio(),context=()=>new (window.AudioContext||window.webkitAudioContext)(),now=()=>performance.now(),schedule=setTimeout,cancel=clearTimeout}={}) {
  ...
  function connect(t){if(!ctx||t.gain)return;try{t.source=ctx.createMediaElementSource(t.audio);t.gain=ctx.createGain();t.gain.gain.value=t.target;t.source.connect(t.gain);t.gain.connect(ctx.destination);if(t.stopTimer){cancel(t.stopTimer);t.stopTimer=null;}t.audio.volume=1;}catch{/* Native volume fallback if Web Audio is unavailable. */}}
  ...
  function level(t,value,ms){if(t.target===value){if(value)start(t);return;}t.target=value;if(t.stopTimer){cancel(t.stopTimer);t.stopTimer=null;}connect(t);if(t.gain){const p=t.gain.gain,time=ctx.currentTime;if(p.cancelAndHoldAtTime)p.cancelAndHoldAtTime(time);else{p.cancelScheduledValues(time);p.setValueAtTime(p.value,time);}p.linearRampToValueAtTime(value,time+ms/1000);}else{
      const from=t.audio.volume,at=now();const fade=()=>{const f=ms?Math.min(1,(now()-at)/ms):1;t.audio.volume=from+(value-from)*f;if(f<1)t.stopTimer=schedule(fade,25);else if(!value)t.audio.pause();};fade();
  ...
  return Object.freeze({
    ...
    dispose(){closed=true;if(pulseTimer)cancel(pulseTimer);for(const t of tracks.values()){if(t.stopTimer)cancel(t.stopTimer);t.audio.pause();t.source?.disconnect();t.gain?.disconnect();}ctx?.close()?.catch(()=>{});},
...
export function enterMusic(){entered=true;controller?.sound(!document.getElementById('soundBtn')?.textContent.includes('OFF'),false);controller?.select('menu');controller?.unlock();}
...
function refresh(){controller?.sound(!document.getElementById('soundBtn')?.textContent.includes('OFF'),false);controller?.select(visibleScene(),{narrating:document.documentElement.dataset.musicNarrating==='true',publicPlague:publicState.plague});}
...
if(typeof document!=='undefined'){
  ...
  window.addEventListener('pageshow',()=>{refresh();if(entered&&document.documentElement.dataset.localHost==='connected')controller.autoplay();});refresh();if(entered&&document.documentElement.dataset.localHost==='connected')controller.autoplay();
}
