...
export function mountLegacyAnimations(size){
 ...
 const realDocument=globalThis.document,document={querySelector:q=>realDocument.querySelector(q),getElementById:id=>realDocument.getElementById(id),body:{appendChild:e=>{if(!disposed)realDocument.body.appendChild(e);return e;}},createElement:tag=>{const e=realDocument.createElement(tag);elements.add(e);const remove=e.remove.bind(e);e.remove=()=>{for(const child of e.querySelectorAll("*"))elements.delete(child);elements.delete(e);remove();};const native=e.animate.bind(e);e.animate=(...args)=>{const a=native(...args);animations.add(a);a.finished.catch(()=>{}).finally(()=>animations.delete(a));return a;};return e;}};
