...
e.style.left=b.x+'px';e.style.top=b.y+'px';e.style.transform='translate(-50%,-50%) rotate(440deg)';return {el:e,key:targetKey};}
async function goblinBombBoom(bomb,targetSide){
if(AUTO_MATCH_MODE) return Promise.resolve();if(!bomb?.el)return Promise.resolve();const p=goblinCellCenter(targetSide,bomb.key),e=document.createElement('div');e.className='goblin-bomb-boom';e.style.left=p.x+'px';e.style.top=p.y+'px';document.body.appendChild(e);bomb.el.remove();
...
{duration:500,easing:'ease-out',fill:'forwards'});try{await an.finished}catch(_){}e.remove();}
...
async function showTransientSalesBlast(...args){logEvent('NEWSFLASH QUEUED',{target:args[0],title:args[1]});try{return await showTransientSalesBlastInner(...args);}finally{if(!disposed)logEvent('NEWSFLASH END',{target:args[0],title:args[1]});}}
