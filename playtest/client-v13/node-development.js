...
export function installNodeDevelopment(session,browserClient){
 ...
 document.addEventListener('keydown',e=>{if(e.ctrlKey&&e.altKey&&!e.shiftKey&&!e.metaKey&&!e.repeat&&(e.code==='Digit6'||e.key==='6')){e.preventDefault();void dev.reveal();}});document.getElementById('playerGrid').addEventListener('click',e=>{if(!takeover)return;const c=e.target.closest('.cell');if(c)void dev.shoot({x:Number(c.dataset.x),y:Number(c.dataset.y)});});
 const bar=document.createElement('aside');bar.style.cssText='position:fixed;bottom:0;left:0;z-index:100000;background:#111;color:white';for(const [label,run]of [['DEV Reveal / Hide (Ctrl+Alt+6)',dev.reveal],['Enemy takeover',dev.takeover]]){const button=document.createElement('button');button.textContent=label;button.onclick=run;bar.append(button);}document.body.append(bar);
}
