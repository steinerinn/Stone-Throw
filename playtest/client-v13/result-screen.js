...
export function buildResultScreen(data,{view,rematch,menu}){
 ...
 for(let place=1;place<=data.count;place++){
  ...
  station.append(people,platform);podium.append(station);
 }
 panel.append(podium);
...
export function mountGroupResult(){
 ...
 function open(data){if(disposed||document.body.classList.contains('st-main-menu-mode'))return;overlay().style.display='none';overlay().classList.remove('cs-result-outro');hide();panel=buildResultScreen(data,{view:hide,rematch:()=>{hide();document.getElementById('stGroupRematch')?.click();},menu:()=>{hide();document.getElementById('stGroupLeave')?.click();}});document.body.append(panel);document.body.classList.add('st-group-results-open');panel.querySelector('button')?.focus();}
