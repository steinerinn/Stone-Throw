...
try{const opened=await session.open();browserClient=await mountStoryBrowser(session.client,{initialConfiguration:opened.configuration,onError:showError,onPublicUpdate:u=>lanClient?.observe(u),additionalBlocked:()=>lanClient?.blocked(),selectMode:mode=>session.request('mode',{mode}),availableModes:opened.modes,initialMode:opened.update?.lan?'multiplayer':undefined,replaceConfiguration:next=>session.configure(next)});lanClient=installLan(session,browserClient,opened);if(opened.reconnected){document.body.classList.remove('st-main-menu-mode');}document.documentElement.dataset.localHost='connected';}
