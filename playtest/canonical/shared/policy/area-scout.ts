...
export function areaScoutChoice(h:HostState,actor:PlayerId,boardId:BoardId,legal:readonly Cell[]):Cell|null {
 const target=h.state.seats.find(s=>s.boardId===boardId);
 if(!target||!legal.length)return null;
 ...
 for(const center of legal){
  ...
  for(let y=Math.max(0,center.y-1);y<=Math.min(h.config.size-1,center.y+1);y++)
   for(let x=Math.max(0,center.x-1);x<=Math.min(h.config.size-1,center.x+1);x++)
    if(!explored.has(x+','+y))fresh++;
  if(fresh>best){best=fresh;pool=[center];}else if(fresh===best)pool.push(center);
 }
