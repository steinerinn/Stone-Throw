import type {UnitType} from '../model.js';import type {ExplicitRng} from '../combat/contracts.js';import {random} from '../combat/rng.js';import * as legality from '../rules/placement.js';import * as geometry from '../rules/index.js';
...
if(roster.assassin){const out=legacyRandomPlacement(profile,SIZE,{...roster,assassin:0},rng,STORY_MODE_ACTIVE,STORY_BATTLE_NUMBER,fixedCells),occupied=new Set([...fixedCells,...out.flatMap(u=>u.cells)]);for(let i=0;i<roster.assassin;i++){const legal=Array.from({length:SIZE*SIZE},(_,i)=>(i%SIZE)+','+Math.floor(i/SIZE)).filter(k=>!occupied.has(k));if(!legal.length)throw Error('No Assassin space');const normal=legal.filter(k=>{const [x,y]=k.split(',').map(Number);return legality.canPlaceInf({size:SIZE,occupied},x!,y!);}),castleCells=out.filter(u=>u.type==='castle').flatMap(u=>u.cells),beside=legal.filter(k=>{const [x,y]=k.split(',').map(Number);return castleCells.some(c=>{const [cx,cy]=c.split(',').map(Number);return Math.abs(x!-cx!)+Math.abs(y!-cy!)===1;});}),preferNormal=random(rng,'placement:assassin-style')<.5,preferred=preferNormal?normal:beside,other=preferNormal?beside:normal,pool=preferred.length?preferred:other.length?other:legal;const k=pool[Math.floor(random(rng,'placement:assassin')*pool.length)]!;out.push({type:'assassin',cells:[k]});occupied.add(k);}return out;}
...
if((roster.elf||0)>1){const out=legacyRandomPlacement(profile,SIZE,{...roster,elf:1},rng,STORY_MODE_ACTIVE,STORY_BATTLE_NUMBER,fixedCells),occupied=new Set([...fixedCells,...out.flatMap(u=>u.cells)]);for(let i=1;i<(roster.elf||0);i++){const legal:string[]=[];for(let y=0;y<SIZE;y++)for(let x=0;x<SIZE;x++)if(legality.canPlaceInf({size:SIZE,occupied},x,y))legal.push(x+','+y);if(!legal.length)throw Error('No legal area Elf placement');const k=legal[Math.floor(random(rng,'placement:area-elf')*legal.length)]!;out.push({type:'elf',cells:[k]});occupied.add(k);}return out;}
...
const playerCanPlaceSingles=(k:string)=>{const p=parseKey(k);return legality.canPlaceInf({size:SIZE,occupied:playerUnits},p.x,p.y);},enemyCanPlaceSingles=(k:string)=>{const p=parseKey(k);return legality.canPlaceInf({size:SIZE,occupied:enemyUnits},p.x,p.y);},playerCanPlaceCavAt=(x:number,y:number,o:'H'|'V')=>legality.canPlaceCav({size:SIZE,occupied:playerUnits},x,y,o).ok,enemyCanPlaceCav=(x:number,y:number,o:'H'|'V')=>legality.canPlaceCav({size:SIZE,occupied:enemyUnits},x,y,o).ok;const sideCastleByCell=(side:string,k:string)=>[...(side==='player'?playerCastles:enemyCastles).values()].find(c=>c.cells.has(k));
function randomConnectedCastleCells(occupiedSet: Set<string>, maxShapeAttempts = 1200) {
    const cellAllowed = (candidate: string, own: Set<string>) => {
    ...
    for (let attempt = 0; attempt < maxShapeAttempts; attempt++) {
        ...
        if (!cellAllowed(start, new Set<string>()))
            continue;
...
function enemyMonkPreferredCell(x: number, y: number) {
    return x >= 2 && x <= SIZE - 3 && y >= 2 && y <= SIZE - 3;
}
function archerAllowedCell(x: number, y: number) {
    return x >= 1 && x <= SIZE - 2 && y >= 1 && y <= SIZE - 2;
}
function clericFarEnoughFromMonk(clericK: string | null, monkK: string | null) {
    if (!clericK || !monkK)
        return true;
...
function nearAnyCells(k: string, cells: Iterable<string>, maxDist = 2) {
    const p = parseKey(k);
    for (const c of cells) {
        const q = parseKey(c);
