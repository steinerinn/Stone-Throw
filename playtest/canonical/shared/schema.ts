import {isArchive} from './archives.js';
/** Exact-key, JSON-only runtime validation. Unknown fields fail closed. */
type S={kind:'str'|'num'|'bool'|'null'}|{kind:'enum';values:readonly unknown[]}|{kind:'array';item:S}|{kind:'object';fields:Record<string,S>}|{kind:'dict';item:S}|{kind:'union';items:S[]};
const str:S={kind:'str'},num:S={kind:'num'},bool:S={kind:'bool'},nil:S={kind:'null'};
const en=(...values:unknown[]):S=>({kind:'enum',values}),arr=(item:S):S=>({kind:'array',item}),obj=(fields:Record<string,S>):S=>({kind:'object',fields}),dict=(item:S):S=>({kind:'dict',item}),union=(...items:S[]):S=>({kind:'union',items}),nullable=(s:S)=>union(nil,s);
const cell=obj({x:num,y:num}),cells=arr(cell),unitType=en('inf','cav','archer','monk','castle','dwarf','goblin','catapult','elf','cleric','demon','dragon','wizard','necro','hero','assassin');
...
function validateInner(schema:S,value:unknown,path='$',ancestors=new Set<object>()):void{
 if(schema.kind==='union'){for(const s of schema.items){if(s.kind==='null'&&value!==null)continue;try{validate(s,value,path,ancestors);return;}catch{}}throw Error(path+': no matching contract');}
 if(schema.kind==='null'){if(value!==null)throw Error(path+': expected null');return;}
 if(schema.kind==='enum'){if(!schema.values.includes(value))throw Error(path+': invalid enum');return;}
 if(schema.kind==='str'){if(typeof value!=='string'||!/^[A-Za-z][A-Za-z0-9_.-]{0,95}$/.test(value)||['constructor','prototype','__proto__'].includes(value))throw Error(path+': invalid token');return;}
 if(schema.kind==='num'){if(typeof value!=='number'||!Number.isSafeInteger(value)||value<0)throw Error(path+': expected nonnegative integer');return;}
 if(schema.kind==='bool'){if(typeof value!=='boolean')throw Error(path+': expected boolean');return;}
 if(value===null||typeof value!=='object'||ancestors.has(value))throw Error(path+': expected acyclic JSON object');
 const next=ancestors;next.add(value);try{
 if(schema.kind==='array'){
  if(!Array.isArray(value))throw Error(path+': expected array');
  if(Reflect.ownKeys(value).length!==value.length+1||Object.keys(value).length!==value.length)throw Error(path+': sparse or extended array');
  for(let i=0;i<value.length;i++){const d=Object.getOwnPropertyDescriptor(value,String(i));if(!d||!('value'in d)||!d.enumerable)throw Error(path+': array accessor');validate(schema.item,d.value,path+'.'+i,next);}return;
 }
 if(Array.isArray(value)||![Object.prototype,null].includes(Object.getPrototypeOf(value)))throw Error(path+': non-JSON object');
 const keys=Reflect.ownKeys(value);for(const key of keys){if(typeof key!=='string')throw Error(path+': symbol key');const d=Object.getOwnPropertyDescriptor(value,key)!;if(!d.enumerable||!('value'in d)||['__proto__','prototype','constructor'].includes(key))throw Error(path+': unsafe property');}
 const record=value as Record<string,unknown>;
 if(schema.kind==='object'){if(keys.length!==Object.keys(schema.fields).length||keys.some(k=>!Object.hasOwn(schema.fields,k)))throw Error(path+': unexpected/missing fields');for(const[k,s]of Object.entries(schema.fields))validate(s,record[k],path+'.'+k,next);}
 else if(schema.kind==='dict'){for(const[k,v]of Object.entries(record)){validate(str,k,path+'.key',next);validate(schema.item,v,path+'.'+k,next);}}
 }finally{next.delete(value);}
}

const validArchives=new WeakMap<object,Set<S>>();
export function validate(schema:S,value:unknown,path='$',ancestors=new Set<object>()):void{const immutable=value!==null&&typeof value==='object'&&isArchive(value);if(immutable&&validArchives.get(value)?.has(schema))return;validateInner(schema,value,path,ancestors);if(immutable){let schemas=validArchives.get(value);if(!schemas){schemas=new Set();validArchives.set(value,schemas);}schemas.add(schema);}}
