...
export function normalEnemyStep(input:HostState,m:NormalMemory,observePresentationStep?:import('../host/lifecycle.js').HostExecution['observePresentationStep']):HostState {let h=structuredClone(input);refreshNormalMemory(h,m);const enemy=h.config.players[1]!,commandId='normal-'+h.history.length;
 if(h.status==='awaiting-turn')return acceptCommand(h,{id:commandId,kind:'advance-turn'},{...normalExecution(m),settleRootBeforeTerminal:true,...(observePresentationStep?{observePresentationStep}:{})});
 if(h.status==='awaiting-command'&&h.activePlayerId===enemy.id){const k=normalPolicy(h,m).target(h.plagueAwareAtTurnStart);if(k)h=acceptCommand(h,{id:commandId,kind:'shoot',actorId:enemy.id,boardId:h.config.players[0]!.boardId,cell:parseKey(k)},{...normalExecution(m),settleRootBeforeTerminal:true,...(observePresentationStep?{observePresentationStep}:{})});else{h.turnStep='plague';h.status='running';pumpHost(h,100000,{...normalExecution(m),settleRootBeforeTerminal:true,...(observePresentationStep?{observePresentationStep}:{})});}return h;}
 if(h.status==='awaiting-decision'){const d=h.pendingRoot!.decisions.find(d=>d.status==='pending')!;if(d.actorId!==enemy.id)throw Error('Normal AI cannot answer human decision');const policy=normalPolicy(h,m);let k:string|null=null;if(d.kind==='hero-relocation'&&h.state.match.units.find(u=>u.id===d.unitId)?.hero?.hitsTaken===1){k=firstHeroRelocation(h,enemy.id,d.legalCells.map(cellKey));}else if(d.kind==='scout'&&d.area){m.scoutQueue=[];const chosen=areaScoutChoice(h,d.actorId,d.boardId,d.legalCells);k=chosen?cellKey(chosen):null;}else if(d.kind==='scout'){if(!m.scoutQueue.length)m.scoutQueue=policy.scout(d.remaining||0);k=m.scoutQueue.shift()||(d.area&&d.legalCells[0]?cellKey(d.legalCells[0]):null);}else if(d.kind==='catapult-target'){k=normalCatapultChoice(h,m);}else if(d.kind==='catapult-roll')k=policy.roll(d.legalCells.map(cellKey),true);else return autoStep(h,{...normalExecution(m),settleRootBeforeTerminal:true,...(observePresentationStep?{observePresentationStep}:{})});return acceptCommand(h,{id:d.kind==='hero-relocation'?'auto-'+(h.history.length+1):commandId,kind:'answer',answer:{actorId:enemy.id,decisionId:d.id,cell:k?parseKey(k):null,unitId:null}},{...normalExecution(m),settleRootBeforeTerminal:true,...(observePresentationStep?{observePresentationStep}:{})});}
 return h;
}
