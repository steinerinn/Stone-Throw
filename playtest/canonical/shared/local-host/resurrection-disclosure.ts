...
export function enemyResurrectionSearch(h:HostState):ResurrectionSearch {
 const enemy=h.config.players[1]!,seat=h.state.seats.find(s=>s.playerId===enemy.id)!;
 if(h.status==='complete'||!seat.resurrection.searchActive)return {active:false,cells:[]};
 const cells=seat.resurrection.suspects.flatMap(id=>{const u=h.state.match.units.find(u=>u.id===id&&u.ownerId===enemy.id);if(!u?.type||!['inf','cav','archer','monk'].includes(u.type))throw Error('Invalid public resurrection casualty');return u.cells.map(cell=>({cell:{...cell},kind:u.type!}));});
 cells.sort((a,b)=>a.cell.y-b.cell.y||a.cell.x-b.cell.x);
 return {active:true,cells};
}
export function maskResurrectionCells(cells:SeenCell[],search:ResurrectionSearch):SeenCell[]{
 if(!search.active)return cells;
 const keys=new Set(search.cells.map(c=>cellKey(c.cell)));
 return [...cells.filter(c=>!keys.has(cellKey(c.cell))),...search.cells.map(c=>({cell:{...c.cell},kind:c.kind,observation:'uncertain' as const}))].sort((a,b)=>a.cell.y-b.cell.y||a.cell.x-b.cell.x);
}
export function knownCasualtyCells(h:HostState,cells:SeenCell[]):SeenCell[]{
 ...
 return cells.map(c=>{if(!c.kind)return c;const u=h.state.match.units.find(u=>u.ownerId===enemy.id&&u.cells.some(p=>cellKey(p)===cellKey(c.cell)));if(!u||!u.cells.every(p=>known.has(cellKey(p))&&seat.shots.includes(cellKey(p))))return c;return {...c,knownDestroyed:true};});
}
...
export function maskResurrectionChoices(cells:Cell[],search:ResurrectionSearch,scouted:string[]):Cell[]{
 if(!search.active)return cells;const map=new Map(cells.map(c=>[cellKey(c),{...c}]));for(const c of search.cells)if(!scouted.includes(cellKey(c.cell)))map.set(cellKey(c.cell),{...c.cell});return [...map.values()].sort((a,b)=>a.y-b.y||a.x-b.x);
}
