...
export function multiCellDisclosure(h:HostState,base:SeenCell[]):SeenCell[]{
 ...
 for(const u of h.state.match.units.filter(u=>u.ownerId===enemy.id&&(u.type==='cav'||u.type==='castle'))){
  ...
  if(u.type==='cav'){if(known.length===u.cells.length)for(const c of known)revealed.add(cellKey(c));}
   ...
   for(const c of known)if(identified||destroyed||known.some(p=>cellKey(p)!==cellKey(c)&&Math.abs(p.x-c.x)<=1&&Math.abs(p.y-c.y)<=1))revealed.add(cellKey(c));
   for(const {event:e} of h.events)if(e.kind==='impact'&&e.unitId===u.id&&e.meta?.targetPlayerId===enemy.id&&e.meta.source==='catapult-shot')for(const c of e.cells)if(knownKeys.has(cellKey(c)))revealed.add(cellKey(c));
  }
