...
export function stationaryDisclosure(h:HostState,base:SeenCell[]):SeenCell[]{
 ...
 for(const {event:e} of h.events)if(e.kind==='impact'&&e.meta?.targetPlayerId===enemy.id){for(const c of e.cells){const k=cellKey(c);if(e.meta.source==='plague')plague.add(k);else hits.set(k,e.unitId);}}
