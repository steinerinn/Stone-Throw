...
export function available(state:CombatState,boardId:BoardId,ownerId:PlayerId):string[]{const n=boardSize(state,boardId),out:string[]=[];for(let y=0;y<n;y++)for(let x=0;x<n;x++){const k=key(x,y);if(!shot(state,ownerId,k))out.push(k);}return out;}
