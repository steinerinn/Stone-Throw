...
export interface CombatSeat {
 ...
 dwarfNow:number;catapultNow:number;catapultLater:number;elfNow:boolean;spyLater:number;areaScoutLater?:number;areaScoutNow?:number;scoutHitCount?:number;
...
export interface CompatPlague {doublePlague?:string;triggerPlayerId:PlayerId;ownerId:PlayerId;targetPlayerId:PlayerId;targetBoardId:BoardId;moveOnPlayerId:PlayerId;origin:Cell|null;outbreaks:Outbreak[];announced:boolean}
export interface RevoltState {qualifying:boolean;rounds:number;level:number;waiting:PlayerId[];started:PlayerId[];pulse?:{resumeStep:'enter'|'finish';level:number;players:PlayerId[];scores:Record<string,{n:string;d:string}>};}
export interface CombatState {
 revolt?:RevoltState;
 ring?:{chaos?:{livingBeforeRoot:boolean;pending:boolean;used:boolean;triggered:boolean};order:PlayerId[];eliminated:PlayerId[];knowledge:Record<string,{scouted:string[];monkCandidates:string[]}>};match:MatchState;seats:CombatSeat[];heroQueue:HeroQueueEntry[];plagues:CompatPlague[];monkDuelActive:boolean;monkDuelHasHappened:boolean;storyMode:boolean;storyPlagueTargets:PlayerId[]}
...
export type Operation=
 |{kind:'revolt-pulse';level:number}
 ...
 |{kind:'turn-scout';ownerId:PlayerId;count:number;area?:boolean}
...
export interface PendingRuleDecision {area?:boolean;id:RuleDecisionId;rootId:ResolutionId;workId:WorkId;actorId:PlayerId;boardId:BoardId;unitId:UnitId|null;kind:'hero-relocation'|'resurrection'|'catapult-target'|'catapult-roll'|'scout'|'policy-target';legalCells:Cell[];legalUnitIds:UnitId[];remaining:number;status:'pending'|'answered'|'cancelled';answer:{cell:Cell|null;unitId:UnitId|null}|null}
...
export interface ResolutionContext {
 ...
 assassinPending?:{meta:SourceMetadata;unitId:UnitId;stage:'activate'|'strike'}[];
