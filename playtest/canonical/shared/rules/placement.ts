...
export function anyConflictsSingle(ctx:Readonly<{occupied:ReadonlySet<string>;size:number}>,targetKey:string,ignoreKeys:ReadonlySet<string>=new Set<string>()){
const around=ring3x3CellsOf(ctx,[targetKey]); const offenders=[]; for(const c of around){ if(ignoreKeys.has(c)) continue; if(ctx.occupied.has(c)) offenders.push(c); } return offenders; }

export function anyConflictsMulti(ctx:Readonly<{occupied:ReadonlySet<string>;size:number}>,targetKeys:readonly string[],ignoreKeys:ReadonlySet<string>=new Set<string>()){
const around=ring3x3CellsOf(ctx,targetKeys); for(const k of targetKeys) around.add(k);const offenders=[]; for(const c of around){ if(ignoreKeys.has(c)) continue; if(ctx.occupied.has(c) && !targetKeys.includes(c)) offenders.push(c); } return offenders; }

export function canPlaceInf(ctx:Readonly<{occupied:ReadonlySet<string>;size:number}>,x:number,y:number){ if(!inBounds(ctx,x,y)) return false; const k=key(x,y); if(ctx.occupied.has(k)) return false; return anyConflictsSingle(ctx,k).length===0; }

export function canPlaceCav(ctx:Readonly<{occupied:ReadonlySet<string>;size:number}>,x:number,y:number,orient:string){ const cells=cavCellsAt(x,y,orient); for(const k of cells){ const {x:cx,y:cy}=parseKey(k); if(!inBounds(ctx,cx,cy)) return {ok:false, offenders:['out']}; } for(const k of cells){ if(ctx.occupied.has(k)) return {ok:false, offenders:[k]}; } const offenders=anyConflictsMulti(ctx,cells); return {ok: offenders.length===0, offenders}; }

export function canPlaceCastleShape(ctx:Readonly<{occupied:ReadonlySet<string>;size:number}>,cells:readonly string[]){
...
if(cellSet.has(k)) continue;if(ctx.occupied.has(k)) offenders.push(k);}
...
export function validCastleExpansionCells(ctx:Readonly<{occupied:ReadonlySet<string>;size:number}>,castle:Readonly<{cells:ReadonlySet<string>}>){
...
if(own.has(ak)) continue;if(ctx.occupied.has(ak)){ blocked=true; break; }
}
