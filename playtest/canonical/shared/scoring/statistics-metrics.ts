...
export function summarizeMatch(descriptor,facts){
 ...
 for(const f of facts){const e=f.event;if(!e)continue;const s=e.statistics||{},m=e.meta,owner=s.environmental?null:m?.ownerId||s.unitOwner,credit=s.environmental?null:human(owner,f.index),type=s.unitType,key=e.rootId+':'+e.workId,chain=f.chainId||e.rootId;
  roots[chain]??=f.originActor||s.rootActorId;
  const processed=s.environmental?0:processedChainCells(e);if(processed){const c=chains[chain]??={actor:roots[chain],index:f.index,cells:0};c.cells+=processed;}
