...
export function calculateScores(d,facts,{expectedEvents=facts.length}={}){
 const accounting={...d,participants:d.participants.map(p=>p.kind==='ai'?{...p,kind:'guest'}:p)},tactics=tacticalScoreFacts(accounting,facts),reasons=[];if(!d.endedAt)reasons.push('not-finalized');
 ...
 for(const f of facts){const e=f.event,s=e.statistics||{},m=e.meta||{};if(s.environmental)continue;const p=d.participants.find(p=>p.actor===m.ownerId),eligible=p&&f.index<(p.cutoff??Infinity);
 ...
 const scores=d.participants.map(p=>{
  ...
  if(p.kind!=='ai'&&!['Full','Quit','Disconnect','Kick','AFK'].includes(p.reliability))issues.push('participation-status-unavailable');
