...
export function tacticalScoreFacts(d,facts){
 ...
 const issue=(actor,index,reason)=>{for(const p of d.participants)if((!actor||p.actor===actor)&&eligible(p.actor,index)&&!result[p.actor].issues.includes(reason))result[p.actor].issues.push(reason);};
 ...
 for(const f of facts){
  ...
  if(s.environmental){if(e.kind==='impact'&&e.unitId)lastImpact.delete(e.unitId);if(['unit-destroyed','hero-killed'].includes(e.kind))dead.add(e.unitId);if(e.kind==='resurrection')dead.delete(e.unitId);continue;}if(seen.has(identity))continue;seen.add(identity);
  if(e.kind==='scouted'){
   const scout=s.scout;
  ...
  if(e.kind==='impact'){
   ...
   for(const p of d.participants)for(const c of e.cells||[])known.add(p.actor+':'+m.targetBoardId+':'+c.x+','+c.y);
   if(m.source==='plague'){
    const owner=s.plague?.owner||m.ownerId;
    if(!owner||!m.targetBoardId||!Array.isArray(e.cells))issue(owner,f.index,'plague-spread-evidence-unavailable');
    else for(const c of e.cells){
     ...
     if(!spread.has(key)&&eligible(owner,f.index))result[owner].plagueCells++;spread.add(key);
    }
   }
  }
  if(e.kind==='resurrection'){dead.delete(e.unitId);lastImpact.delete(e.unitId);}
  if(e.kind==='unit-destroyed'||e.kind==='hero-killed'){
   ...
   if(!e.unitId||!cause?.ownerId||!cause.source||!s.unitOwner){issue(cause?.ownerId,f.index,'special-kill-causality-unavailable');continue;}
   if(dead.has(e.unitId))continue;dead.add(e.unitId);
   if(direct.has(cause.source))continue;
   if(!abilities.has(cause.source)){issue(cause.ownerId,f.index,'special-kill-causality-unavailable');continue;}
   const owner=cause.source==='plague'?(s.plague?.owner||cause.ownerId):cause.ownerId;
   if(owner!==s.unitOwner&&eligible(owner,f.index)&&(!hit||eligible(owner,hit.index)))result[owner].kills++;
  }
 }
 return result;
}
