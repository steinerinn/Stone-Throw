...
const validArchives = new WeakSet();
...
function jsonOnly(value, seen = new Set()) { if (value && typeof value === 'object' && validArchives.has(value))
    return; if (value === null || typeof value === 'string' || typeof value === 'boolean')
    ...
    throw Error('Host contains symbols'); seen.add(value); for (const key of Object.keys(value)) {
    const descriptor = Object.getOwnPropertyDescriptor(value, key);
    if (descriptor.get || descriptor.set)
        throw Error('Host contains hidden/accessor state');
    jsonOnly(descriptor.value, seen);
} if (Object.getOwnPropertyNames(value).length !== Object.keys(value).length + (Array.isArray(value) ? 1 : 0))
    throw Error('Host contains hidden/accessor state'); if (Array.isArray(value))
    for (let i = 0; i < value.length; i++)
        if (!Object.hasOwn(value, i))
            throw Error('Sparse host array'); seen.delete(value); if (isArchive(value))
    validArchives.add(value); }
