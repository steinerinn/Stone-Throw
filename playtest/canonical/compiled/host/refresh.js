...
export function observeResurrectionFeedback(host, boardId, cell, found) { for (const observer of host.state.match.players) {
    const k = host.state.match.knowledge[observer.id];
    (k.events = mutableRows(k.events)).push({ sequence: k.events.length + 1, kind: found ? 'resurrection-found' : 'resurrection-rejected', boardId, cell: { ...cell }, unitType: null, contactId: null });
} }
export function observeRuleEvent(host, event) {
    ...
    if (event.kind === 'impact' && meta) {
        ...
        for (const c of event.cells) {
            ...
            for (const observer of m.players) {
                ...
                if (board) {
                    board.cells = mutableRows(board.cells);
                    const old = board.cells.find(x => cellKey(x.cell) === k);
                    if (old) {
                        board.cells[board.cells.indexOf(old)] = { ...old, observation: event.unitId ? 'impact' : 'miss' };
                    }
                    else
                        board.cells.push({ cell: { ...c }, observation: event.unitId ? 'impact' : 'miss', unitType: null, contactId: null });
                }
                (known.events = mutableRows(known.events)).push({ sequence: known.events.length + 1, kind: event.unitId ? 'impact' : 'miss', boardId: meta.targetBoardId, cell: { ...c }, unitType: null, contactId: null });
            }
        }
    }
    if ((event.kind === 'resurrection' || event.kind === 'hero-moved') && event.unitId) {
        const u = m.units.find(u => u.id === event.unitId);
        for (const c of event.cells)
            (m.history = mutableRows(m.history)).push({ id: id('event-' + (m.history.length + 1)), kind: event.kind === 'resurrection' ? 'damage-restored' : 'relocation', actorId: u.ownerId, targetId: u.ownerId, boardId: u.boardId, unitId: u.id, cell: { ...c }, turnIndex: host.turnIndex });
        for (const observer of m.players)
            (m.knowledge[observer.id].events = mutableRows(m.knowledge[observer.id].events)).push({ sequence: m.knowledge[observer.id].events.length + 1, kind: event.kind === 'resurrection' ? 'resurrection-announced' : 'hero-moved', boardId: u.boardId, cell: null, unitType: null, contactId: null });
    }
