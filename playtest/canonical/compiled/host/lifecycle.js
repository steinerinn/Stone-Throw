...
import { answerDecision } from '../combat/decisions.js';
import { random, shuffle } from '../combat/rng.js';
...
export function pumpHost(h, maxSteps = 100000, execution) {
    let steps = 0;
    while (h.status === 'running') {
        if (++steps > maxSteps)
            throw Error('Host step bound exceeded');
        if (h.pendingRoot) {
            ...
            if (r.status === 'complete') {
                ...
                const p = seat(h.state, h.activePlayerId);
                if (purpose === 'shot') {
                    if (p.sameTurnBonusTarget === 'chain') {
                        p.ordinaryShots += p.currentChainBonus;
                        p.currentChainBonus = 0;
                    }
                    h.turnStep = p.ordinaryShots > 0 ? 'actions' : 'plague';
                }
        ...
        if (h.turnStep === 'enter') {
            ...
            if (primary) {
                ...
                h.turnStep = 'resurrection';
        }
        else if (h.turnStep === 'resurrection') {
            h.turnStep = primary ? 'scout' : 'scout';
            if (p.clericLater)
                root(h, [{ kind: 'turn-resurrection', ownerId: p.playerId }], 'entry');
        }
            ...
            if (p.catapultLater) {
                const count = p.catapultLater;
                p.catapultLater = 0;
                root(h, [{ kind: 'catapult-series', ownerId: p.playerId, remaining: count }], 'entry');
            }
        }
        else if (h.turnStep === 'budget') {
            budget(h);
            h.turnStep = 'actions';
        }
        else if (h.turnStep === 'actions') {
            if (p.ordinaryShots <= 0) {
                h.turnStep = 'plague';
                continue;
            }
            if (!primary) {
                const probe = startResolution(h.state, 'match-root-' + (h.rootSerial + 1), 'host-root-' + (h.rootSerial + 1), p.playerId, [], h.rng);
                if (checkTerminal(probe, 'enemy-pre-shot')) {
                    h.rootSerial++;
                    h.pendingRoot = probe;
                    h.rootEventCursor = 0;
                    collect(h);
        ...
        else if (h.turnStep === 'plague') {
            if (h.lastProbeSkippedPlague) {
                h.turnStep = 'finish';
                continue;
            }
...
export function acceptCommand(host, command, execution) {
    if (!['place', 'start', 'advance-turn', 'shoot', 'answer'].includes(command.kind))
        throw Error('Unknown host command');
    if (host.status === 'complete')
        throw Error('Match already complete');
    if (host.history.some(r => r.command.id === command.id))
        throw Error('Duplicate command');
    if (!/^[A-Za-z0-9_-]{1,96}$/.test(command.id))
        throw Error('Invalid command ID');
    const h = structuredClone(host), rngBefore = h.rng.cursor, eventStart = h.events.length;
    if (command.kind === 'place')
        ...
        for (const p of h.config.players)
            for (const [type, count] of Object.entries(p.roster))
                if (h.state.match.units.filter(u => u.ownerId === p.id && u.type === type).length !== count)
                    throw Error('Incomplete roster');
        h.initial = structuredClone(h.state);
        h.initialRng = structuredClone(h.rng);
        for (const u of h.state.match.units)
            if (u.hero)
                u.hero.originalCell = u.hero.currentCell ? { ...u.hero.currentCell } : null;
        if (h.state.ring) {
            shuffle(h.state.ring.order, h.rng, 'initial-active-ring');
            syncRing(h.state);
            h.starterId = h.state.ring.order[0];
        }
        else
            h.starterId = h.config.players[Math.floor(random(h.rng, 'round-starter') * h.config.players.length)].id;
        h.activePlayerId = h.starterId;
        h.round = 1;
        h.turnIndex = 1;
        h.status = 'running';
        h.turnStep = 'enter';
    ...
    else if (command.kind === 'advance-turn') {
        if (h.status !== 'awaiting-turn')
            throw Error('No turn boundary');
        h.status = 'running';
        pumpHost(h, 100000, execution);
    }
    else if (command.kind === 'answer') {
        if (h.status !== 'awaiting-decision' || !h.pendingRoot)
            throw Error('No decision');
        answerDecision(h.pendingRoot, command.answer);
        collect(h);
        h.status = h.pendingRoot.status === 'awaiting-decision' ? 'awaiting-decision' : 'running';
        pumpHost(h, 100000, execution);
    }
    else {
        if (h.status !== 'awaiting-command' || command.actorId !== h.activePlayerId)
            throw Error('Wrong actor or phase');
        const p = seat(h.state, command.actorId), target = normalTarget(h.state, p.playerId);
        if (command.boardId !== target.boardId || command.cell.x < 0 || command.cell.y < 0 || command.cell.x >= h.config.size || command.cell.y >= h.config.size || !Number.isInteger(command.cell.x) || !Number.isInteger(command.cell.y))
            throw Error('Illegal target');
        const k = cellKey(command.cell), repeated = target.shots.includes(k), probe = target.resurrection.searchActive && target.resurrection.suspects.some(id => h.state.match.units.find(u => u.id === id)?.cells.some(c => cellKey(c) === k));
        if (repeated && !probe)
            throw Error('Already shot');
        const stats = h.counters.find(s => s.playerId === p.playerId);
        stats.shots++;
        stats.directHits += !repeated && target.occupied.includes(k) ? 1 : 0;
        stats.cellsAffected++;
        stats.biggestAttack = Math.max(1, stats.biggestAttack);
        h.guards.turnActions++;
        if (repeated) {
            const suspect = target.resurrection.suspects.find(id => h.state.match.units.find(u => u.id === id)?.cells.some(c => cellKey(c) === k));
            target.resurrection.suspects = target.resurrection.suspects.filter(id => id !== suspect);
            observeResurrectionFeedback(h, target.boardId, command.cell, false);
            p.ordinaryShots--;
            h.lastProbeSkippedPlague = p.sameTurnBonusTarget === 'ordinary';
            h.turnStep = p.ordinaryShots > 0 ? 'actions' : 'plague';
            h.status = 'running';
            pumpHost(h, 100000, execution);
        }
        else {
            h.lastProbeSkippedPlague = false;
            root(h, [{ kind: 'host-direct-shot', meta: { actorId: p.playerId, ownerId: p.playerId, targetPlayerId: target.playerId, targetBoardId: target.boardId, sourceUnitId: null, source: p.sameTurnBonusTarget === 'ordinary' ? 'direct-human' : 'direct-ai', origin: { ...command.cell } }, cell: { ...command.cell } }, { kind: 'terminal-check', reason: 'direct-end' }], 'shot');
            pumpHost(h, 100000, execution);
        }
    }
