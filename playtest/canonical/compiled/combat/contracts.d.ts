import type { Id, MatchState, PlayerId, BoardId, UnitId, Cell } from '../model.js';
...
export interface ExplicitRng {
    algorithm: 'lcg32-with-tape';
    seed: number;
    state: number;
    cursor: number;
    tape: number[];
...
export interface CombatSeat {
    ...
    releaseRevivesPendingFirst: boolean;
    occupied: string[];
    shots: string[];
    scouted: string[];
...
export interface CombatState {
    ...
    match: MatchState;
    seats: CombatSeat[];
    heroQueue: HeroQueueEntry[];
    plagues: CompatPlague[];
    monkDuelActive: boolean;
    monkDuelHasHappened: boolean;
    storyMode: boolean;
    storyPlagueTargets: PlayerId[];
}
...
export type Operation = {
    ...
    kind: 'monk-continuation';
    meta: SourceMetadata;
} | {
    kind: 'scheduled-benefit';
    ownerId: PlayerId;
    benefit: 'dwarf' | 'catapult' | 'scout' | 'resurrection' | 'ordinary-shots';
    amount: number;
    meta: SourceMetadata;
    accounting: 'already-applied-to-compatibility-counters';
} | {
    kind: 'direct-shot';
    meta: SourceMetadata;
    cell: Cell;
    ...
    cell: Cell;
    impact: number;
    generated: ReactionEntry[];
} | {
    kind: 'catapult-resume';
    meta: SourceMetadata;
    impact: number;
    generated: ReactionEntry[];
    decisionId: RuleDecisionId;
} | {
    kind: 'catapult-series';
    ownerId: PlayerId;
    remaining: number;
} | {
    kind: 'plague-step';
    ...
    nextFrontier: string[];
    reserved: string[];
    stage: 'start' | 'branch' | 'child' | 'finish';
} | {
    kind: 'terminal-check';
    reason: 'chain-end' | 'direct-end' | 'catapult-sibling' | 'plague-sibling';
};
export interface RuleWork {
    id: WorkId;
    rootId: ResolutionId;
...
export interface AttackFrame {
    detachedPlague: CompatPlague | null;
    compatibilityTurnId: PlayerId;
    id: WorkId;
    kind: 'root' | 'attack' | 'wave' | 'interrupt' | 'catapult' | 'plague';
    current: RuleWork[];
    next: ReactionEntry[];
    deferred: {
        unitId: UnitId;
...
export interface PendingRuleDecision {
    ...
    boardId: BoardId;
    unitId: UnitId | null;
    kind: 'hero-relocation' | 'resurrection' | 'catapult-target' | 'catapult-roll' | 'scout' | 'policy-target';
    legalCells: Cell[];
    legalUnitIds: UnitId[];
    remaining: number;
...
export interface ResolutionContext {
    ...
    events: InternalRuleEvent[];
    generated: ReactionEntry[];
    externalEntropy: {
        beforeImpact: number;
        purpose: 'legacy-visual' | 'legacy-sound';
        values: number[];
    }[];
    nextWork: number;
    nextDecision: number;
    completedAtEvent: number | null;
}
