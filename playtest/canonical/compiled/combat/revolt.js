...
export function beginRevoltPulse(ctx, level) {
    const pulse = ctx.state.revolt?.pulse;
    if (!pulse || pulse.level !== level)
        throw Error('Missing Revolt pulse');
    ctx.environmental = { kind: 'peasant-revolt', level };
    emit(ctx, 'peasant-revolt', null, null, [], level, 'round-complete');
    const ops = [];
    for (const id of pulse.players) {
        const p = seat(ctx.state, id), size = ctx.state.match.boards.find(b => b.id === p.boardId).width, cells = [];
        ...
        for (let i = 0; i < level && cells.length; i++) {
            const [k] = cells.splice(Math.floor(random(ctx.rng, 'peasant-revolt-target') * cells.length), 1), cell = parseKey(k);
    ...
    frame.current.splice(frame.cursor, 0, ...ops.map(op => work(ctx, op, 'turn-boundary')));
}
