...
function ensure(ok, message) { if (!ok)
    throw Error(message); }
function plain(v, seen = new Set()) { if (v === null || typeof v === 'boolean')
    return; if (typeof v === 'string') {
    ensure(!/<\/?[a-z][^>]*>/i.test(v), 'HTML is not rule state');
    return;
} if (typeof v === 'number') {
    ensure(Number.isFinite(v), 'Nonfinite rule state');
    return;
} ensure(v && typeof v === 'object', 'Non-JSON rule state'); ensure(!seen.has(v), 'Cyclic rule state'); seen.add(v); ensure(Object.getOwnPropertySymbols(v).length === 0, 'Symbol rule state'); const descriptors = Object.getOwnPropertyDescriptors(v); for (const [k, d] of Object.entries(descriptors)) {
    ensure(!d.get && !d.set, 'Accessor rule state');
    if (k !== 'length' || !Array.isArray(v))
        ensure(d.enumerable, 'Hidden rule state');
} if (Array.isArray(v)) {
    for (let i = 0; i < v.length; i++) {
        ensure(i in v, 'Sparse rule array');
        plain(v[i], seen);
    }
}
else {
    ensure(Object.getPrototypeOf(v) === Object.prototype || Object.getPrototypeOf(v) === null, 'Nonplain rule state');
    for (const value of Object.values(v))
        plain(value, seen);
} seen.delete(v); }
function exact(v, keys) { ensure(Object.keys(v).sort().join('|') === [...keys].sort().join('|'), 'Unknown or missing fields'); }
...
export function assertResolution(value) {
    ...
    if (ctx.state.ring) {
        ...
        for (const [key, value] of Object.entries(ctx.state.ring.knowledge)) {
            ...
            for (const list of [value.scouted, value.monkCandidates])
                ensure(Array.isArray(list) && list.every(k => typeof k === 'string' && /^\d+,\d+$/.test(k)), 'Invalid ring knowledge cells');
        }
        ;
        const all = ctx.state.match.players.map(p => p.id), ids = [...ctx.state.ring.order, ...ctx.state.ring.eliminated];
        ensure(ids.length === all.length && new Set(ids).size === all.length && ids.every(p => all.includes(p)), 'Invalid ring membership');
    }
    assertMatchState(ctx.state.match);
    const state = ctx.state;
    const players = new Set(state.match.players.map(p => p.id)), boards = new Map(state.match.boards.map(b => [b.id, b])), units = new Set(state.match.units.map(u => u.id));
    ...
    for (const p of state.seats) {
        ...
        for (const list of [p.occupied, p.shots, p.scouted, p.monkCandidates, p.plagueExcluded]) {
            ensure(Array.isArray(list) && new Set(list).size === list.length, 'Duplicate compatibility cells');
            for (const k of list) {
                ensure(typeof k === 'string' && /^\d+,\d+$/.test(k), 'Invalid cell key');
                const [x, y] = k.split(',').map(Number), b = boards.get(p.boardId);
                ensure(x < b.width && y < b.height, 'Cell outside board');
            }
        }
    }
    ...
    for (const frame of ctx.frames) {
        ...
        ids.add(frame.id);
        ensure(integer(frame.cursor) && frame.cursor <= frame.current.length, 'Invalid continuation cursor');
        for (const w of frame.current) {
            exact(w, ['id', 'rootId', 'parentId', 'timing', 'operation']);
            ensure(w.rootId === ctx.id && !ids.has(w.id), 'Invalid work identity');
            ids.add(w.id);
            ensure(['immediate-interrupt', 'attack-remainder', 'after-attack', 'next-wave', 'same-turn', 'turn-boundary', 'future-turn'].includes(w.timing), 'Unknown timing');
            op(w.operation);
        }
        for (const d of frame.deferred) {
            ensure(units.has(d.unitId), 'Unknown deferred unit');
            meta(d.meta);
        }
    }
    for (const w of ctx.future) {
        exact(w, ['id', 'rootId', 'parentId', 'timing', 'operation']);
        ensure(w.rootId === ctx.id && !ids.has(w.id) && w.timing === 'future-turn', 'Invalid future work');
        ids.add(w.id);
        op(w.operation);
        ensure(w.operation.kind === 'scheduled-benefit' && w.operation.accounting === 'already-applied-to-compatibility-counters', 'Invalid future accounting');
    }
    for (const e of ctx.generated)
        meta(e.meta);
    ensure(integer(ctx.nextWork) && integer(ctx.nextDecision), 'Invalid identity counters');
    ensure(ctx.rng.algorithm === 'lcg32-with-tape' && integer(ctx.rng.cursor) && ctx.rng.cursor === ctx.rng.draws.length, 'Invalid RNG state');
    ensure(integer(ctx.rng.state) && ctx.rng.state <= 0xffffffff, 'Invalid RNG word');
    for (const n of ctx.rng.tape)
        ensure(n >= 0 && n < 1, 'Invalid RNG tape');
    ctx.rng.draws.forEach((d, i) => ensure(d.ordinal === i && d.value >= 0 && d.value < 1, 'Invalid RNG journal'));
    let rngWord = ctx.rng.seed;
    ensure(integer(rngWord) && rngWord <= 0xffffffff, 'Invalid RNG seed');
    for (let i = 0; i < ctx.rng.cursor; i++) {
        rngWord = (Math.imul(1664525, rngWord) + 1013904223) >>> 0;
        const expected = i < ctx.rng.tape.length ? ctx.rng.tape[i] : rngWord / 4294967296;
        ensure(ctx.rng.draws[i].value === expected, 'RNG replay mismatch');
    }
    ...
    for (const d of ctx.decisions) {
        ...
        for (const c of d.legalCells) {
            exact(c, ['x', 'y']);
            const b = boards.get(d.boardId);
            ensure(b && integer(c.x) && integer(c.y) && c.x < b.width && c.y < b.height, 'Illegal decision geometry');
        }
        ensure(d.rootId === ctx.id && players.has(d.actorId) && boards.has(d.boardId), 'Invalid decision scope');
        ensure(!decisionIds.has(d.id), 'Duplicate decision');
        decisionIds.add(d.id);
        ensure(d.status === 'pending' || d.status === 'answered' || d.status === 'cancelled', 'Invalid decision status');
        ensure(d.unitId === null || units.has(d.unitId), 'Unknown decision unit');
        for (const u of d.legalUnitIds)
    ...
    ensure(ctx.status === 'complete' ? ctx.completedAtEvent === completed[0]?.sequence && ctx.frames.length === 0 : ctx.completedAtEvent === null, 'Invalid completion marker');
}
export function serializeResolution(ctx) { assertResolution(ctx); return stableJson(ctx); }
export function deserializeResolution(text) { ensure(text.length <= 32 * 1024 * 1024, 'Resolution too large'); const value = JSON.parse(text); assertResolution(value); return value; }
