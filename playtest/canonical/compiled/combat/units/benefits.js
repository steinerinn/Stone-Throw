...
export function elfRule(ctx, meta) { const p = seat(ctx.state, meta.targetPlayerId); if (meta.source === 'plague')
    return; const pacing = ctx.state.match.config.rulesVersion === 'stone-throw-pacing-v1'; const area = pacing && (p.scoutHitCount || 0) > 0; if (pacing)
    p.scoutHitCount = (p.scoutHitCount || 0) + 1; if (area) {
    if (!meta.source.startsWith('direct-') && p.playerId === ruleTurn(ctx))
        p.areaScoutNow = (p.areaScoutNow || 0) + 1;
    else {
        p.areaScoutLater = (p.areaScoutLater || 0) + 1;
        futureBenefit(ctx, meta, 'scout', 1);
    }
    emit(ctx, 'benefit-scheduled', meta, null, [], 1, 'area-scout');
    return;
} if (!meta.source.startsWith('direct-') && p.playerId === ruleTurn(ctx))
    p.elfNow = true;
else
