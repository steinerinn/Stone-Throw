...
function validateInner(schema, value, path = '$', ancestors = new Set()) {
    ...
    try {
        ...
        for (const key of keys) {
            ...
            if (!d.enumerable || !('value' in d) || ['__proto__', 'prototype', 'constructor'].includes(key))
                throw Error(path + ': unsafe property');
        }
        const record = value;
        if (schema.kind === 'object') {
            if (keys.length !== Object.keys(schema.fields).length || keys.some(k => !Object.hasOwn(schema.fields, k)))
...
const validArchives = new WeakMap();
export function validate(schema, value, path = '$', ancestors = new Set()) { const immutable = value !== null && typeof value === 'object' && isArchive(value); if (immutable && validArchives.get(value)?.has(schema))
    return; validateInner(schema, value, path, ancestors); if (immutable) {
    let schemas = validArchives.get(value);
    if (!schemas) {
        schemas = new Set();
        validArchives.set(value, schemas);
    }
    schemas.add(schema);
} }
