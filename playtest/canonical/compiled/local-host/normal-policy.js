...
export function normalPolicy(h, memory) {
    ...
    const targeting = avoided.size ? createNormalPolicy({ ...env, enemyShots: new Set([...shots, ...avoided]), alreadyShotOnSide: (_side, k) => shots.has(k) || avoided.has(k) }, () => random(h.rng, 'normal-browser-policy')) : legacy;
    return { plagueDiagnostic: (selected) => ({ selected, protectedCells: [...legacy.protectedCells()], finalCoreException: !!selected && !!groupPlagueFinalCoreCell?.(selected), remainingCore: survivingCore.length, activatedHeroSurvives: !!(hero?.hero?.activated && hero.hero.currentCell && hero.hero.hitsTaken < 3) }), target: (plagueAware) => plagueAware ? targeting.plagueTarget() : targeting.target(), catapult: (avoid, origins) => legacy.catapult(avoid, origins), roll: (cells, protect = false) => legacy.catapultRoll(cells, protect ? legacy.protectedCells() : new Set()), scout: (count) => { legacy.scout(count); return picked; } };
}
