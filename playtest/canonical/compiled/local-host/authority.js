import { publicScoutOnlyCells } from './scout-visuals.js';
import { publicPlagueActive, resolvedStoryPlagueTargets } from './plague-presentation.js';
...
export function createLocalAuthority(config, checkpoint, development = false) {
    ...
    function applyPlace(owner, unit, type, cells, scoutMode) { h = acceptCommand(h, { id: commandId(), kind: 'place', placement: { unitId: id(unit), ownerId: owner.id, boardId: owner.boardId, type, cells, ...(scoutMode ? { scoutMode } : {}) } }); }
    ...
    async function execute(intent, onPlayerFrame) {
        ...
        if (intent.kind === 'place')
            ...
            if (intent.kind === 'move')
                applyPlace(self, unit.unitId, unit.type, intent.cells, unit.scoutMode);
        }
