...
export function createLocalSession(config, checkpoint, development = false) {
    ...
    const client = Object.freeze({
        ...
        dispatch: (request) => ordered(() => current.client.dispatch(request))
    });
    return Object.freeze({ ...(development ? { dev: Object.freeze({ inspect: () => ordered(() => current.dev.inspect()), takeover: (enabled) => ordered(() => current.dev.takeover(enabled)), shoot: (cell) => ordered(() => current.dev.shoot(cell)) }) } : {}), client,
        ...
        configure: (next) => ordered(async () => {
            ...
            host.initialRng = structuredClone(old.rng);
            refreshHost(host);
            host.initial = structuredClone(host.state);
            ...
            const update = await replacement.client.read();
            current = replacement;
            return update;
        })
    });
