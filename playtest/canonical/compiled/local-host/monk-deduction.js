...
export function updateMonkEvidence(size, prior, probe, deflected) {
...
export function deduceMonkCandidates(size, evidence, seen) {
    // A publicly identified Monk is no longer a question, alive or dead.
    if (seen.some(c => c.kind === 'monk'))
        return [];
    ...
    for (const c of seen) {
        ...
        if (hit || occupied || c.observation === 'empty')
            impossible.add(key(c.cell));
        ...
        if (occupied && c.kind !== null && c.kind !== 'hero' || c.corePresentation === 'unidentified')
            for (const k of neighbors8({ size }, c.cell.x, c.cell.y))
                impossible.add(k);
    }
