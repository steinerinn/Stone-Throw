...
export function animationCue(h, events, groups) {
    const e = events.find(e => e.kind === 'impact' && e.meta && ['direct-human', 'direct-ai', 'catapult-shot', 'wizard', 'archer', 'goblin', 'demon-blast', 'dragon', 'assassin'].includes(e.meta.source));
    if (!e?.meta || !e.cells[0])
        return;
...
export function impactSources(events, delta, selfBoard, opponentBoard) {
    const sources = { 'direct-human': 'direct', 'direct-ai': 'direct', archer: 'archer', 'catapult-shot': 'catapult', goblin: 'goblin', wizard: 'wizard', dragon: 'dragon', 'demon-blast': 'demon', plague: 'plague', 'monk-deflect': 'monk', assassin: 'assassin' };
