...
export function normalCatapultChoice(h, memory) {
    const root = h.pendingRoot, d = root?.decisions.find(d => d.status === 'pending');
    if (!root || !d || d.kind !== 'catapult-target')
        throw Error('No normal Catapult target decision');
    const stopped = new Set();
    for (const e of root.events) {
        if (e.workId !== d.workId || e.kind !== 'impact' || e.meta?.source !== 'catapult-shot' || e.meta.ownerId !== d.actorId)
            continue;
        if (root.state.match.units.find(u => u.id === e.unitId)?.type === 'castle')
            for (const cell of e.cells)
                stopped.add(cellKey(cell));
    }
