...
export function publicUnitStrips(h) {
    ...
    for (const [i, p] of h.config.players.entries()) {
        ...
        for (const [type, count] of Object.entries(p.roster)) {
            ...
            result[side][type] = { placed: i === 0 ? own.length : units.length ? count : 0, destroyed: type === 'hero' ? own.filter(u => u.lifecycle === 'destroyed').length : own.filter(u => u.cells.length && u.cells.every(c => shots.has(cellKey(c)))).length, heroHits: type === 'hero' ? (own[0]?.hero?.hitsTaken || 0) : 0 };
        }
    }
    return result;
}
