...
export async function mountStoryBrowser(transport,{replaceConfiguration,initialConfiguration,onError,onPublicUpdate,additionalBlocked}){
 ...
 const callbacks={onError,playback,externalSession:true,inputLocked:()=>busy||tutorials.open(),blocked:()=>busy||paused||tutorials.open()||additionalBlocked?.(),configuration:()=>active,
  ...
  async onUpdate(update,view){
   onPublicUpdate?.(update);lastSnapshot=update.snapshot;policy.observePhase(view.phase);await tutorials.wait();
 ...
 return Object.freeze({apply:update=>renderer.apply(update),ready:()=>replacement,remount:async()=>{renderer.unmount();renderer=await mountClient(transport,callbacks);},unmount:()=>renderer.unmount()});
}
