...
for(const n of [2,3,4])for(const seed of [1,2,3,42,99]){
 ...
 assert.ok(ctx.events.some(e=>e.kind==='monk-duel'&&e.reason==='defeated'));for(let i=2;i<n;i++)assert.equal(ctx.state.seats[i].shots.length,0,'third/fourth seat untouched');checks++;
}
