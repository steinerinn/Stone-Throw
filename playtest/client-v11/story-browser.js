...
export async function mountStoryBrowser(transport,{replaceConfiguration,initialConfiguration,onError}){
 const tutorials=installStoryTutorials(),playback={pending:null};let renderer,policy,active={size:15,story:false,battle:0},replacement=Promise.resolve(),busy=false,paused=false,lastSnapshot=null;
 const status=text=>{document.getElementById('status').textContent=text;};
 async function replace(next){
  busy=true;
  try{await replaceConfiguration(next);paused=false;active=structuredClone(next);playback.pending=null;playback.startedAt=null;playback.endedAt=null;tutorials.reset(active);if(renderer)renderer.unmount();renderer=await mountClient(transport,callbacks);}
  finally{busy=false;const shots=document.getElementById('shotsLeft');shots.textContent=shots.textContent;}
 }
 const schedule=next=>{const task=replacement.then(()=>replace(next));replacement=task.catch(error=>{console.error('Story battle replacement failed',error);});return task;};
 const callbacks={onError,playback,externalSession:true,inputLocked:()=>busy||tutorials.open(),blocked:()=>busy||paused||tutorials.open(),configuration:()=>active,
  reset:()=>window.__stoneThrowResetFullGame(),playAgain:()=>resultButtonClick(),
  async onUpdate(update,view){
   lastSnapshot=update.snapshot;policy.observePhase(view.phase);await tutorials.wait();
 ...
 window.__stoneThrowResumeStoryRuntime=()=>{paused=false;renderer.remount();return true;};
 ...
 renderer=await mountClient(transport,callbacks);
 return Object.freeze({ready:()=>replacement,remount:async()=>{renderer.unmount();renderer=await mountClient(transport,callbacks);},unmount:()=>renderer.unmount()});
}
