...
const session=createHttpSession();let browserClient;
try{const opened=await session.open();browserClient=await mountStoryBrowser(session.client,{initialConfiguration:opened.configuration,onError:showError,replaceConfiguration:next=>session.configure(next)});if(opened.reconnected){document.body.classList.remove('st-main-menu-mode');}document.documentElement.dataset.localHost='connected';}
