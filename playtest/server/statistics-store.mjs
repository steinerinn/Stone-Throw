...
export function migrateStatistics(db,directory){
 const version=Number(db.prepare('PRAGMA user_version').get().user_version);if(version>2)throw Error('Unsupported Registry schema');if(version===2){if(db.prepare('PRAGMA table_info(stat_global)').all().some(c=>c.name==='unit_cells'))return null;const backups=path.join(directory,'backups');fs.mkdirSync(backups,{recursive:true,mode:0o700});const file=path.join(backups,'registry-before-unit-cells-'+randomUUID()+'.sqlite');db.prepare('VACUUM INTO ?').run(file);db.exec('ALTER TABLE stat_global ADD COLUMN unit_cells INTEGER NOT NULL DEFAULT 0');return file;}
 ...
 CREATE TABLE IF NOT EXISTS stat_global(id INTEGER PRIMARY KEY CHECK(id=1),shots INTEGER NOT NULL DEFAULT 0,cells INTEGER NOT NULL DEFAULT 0,plague INTEGER NOT NULL DEFAULT 0,scouted INTEGER NOT NULL DEFAULT 0,unit_cells INTEGER NOT NULL DEFAULT 0);
...
export function statisticsStore(db,{onFinalized=()=>{}}={}){
 ...
 function capture(d,h){if(!d||d.mode==='Story')return;const old=q('SELECT event_cursor,command_cursor,finalized,descriptor FROM stat_matches WHERE id=?',d.id),json=JSON.stringify(d);if(old?.finalized)return;if(old&&old.event_cursor===h.events.length&&old.command_cursor===h.history.length&&old.descriptor===json)return;
  tx(()=>{run('INSERT OR IGNORE INTO stat_matches(id,mode,player_count,rules_version,build,started_at,descriptor) VALUES(?,?,?,?,?,?,?)',d.id,d.mode,d.participants.length,h.config.rulesVersion,d.build,d.startedAt,json);
   ...
   captureReplayCheckpoint(db,d,h);
   if(!d.endedAt)return;
   ...
   for(const p of d.participants){const s=summary[p.actor];run('INSERT INTO stat_participants(match_id,actor,player_id,kind,seat,outcome,reliability,placement,summary) VALUES(?,?,?,?,?,?,?,?,?)',d.id,p.actor,p.playerId||null,p.kind,p.seat,s.outcome||null,s.reliability||null,s.placement,JSON.stringify(s));if(p.kind==='ai')continue;
    run('UPDATE stat_global SET shots=shots+?,cells=cells+?,plague=plague+?,scouted=scouted+?,unit_cells=unit_cells+? WHERE id=1',s.shots,s.destructiveCells,s.plagueCells,s.scoutFound,s.unitCellsHit);
   ...
   persistScores(db,d,facts);
   finalizeReplay(db,d,facts);
 ...
 const rows=playerId?all("SELECT p.match_id,p.reliability,p.seat,m.descriptor FROM stat_participants p JOIN stat_matches m ON m.id=p.match_id WHERE p.player_id=? AND p.kind='account' AND m.finalized=1 AND m.mode IN ('Duel','3 Players','4 Players') ORDER BY m.ended_at,m.id",playerId):[];
 return reliabilityHistory(rows);
 },hallOfFame(mode,viewerId,options){const rows=all("SELECT c.player_id,c.value,a.display_name,a.country FROM stat_career c JOIN accounts a ON a.id=c.player_id WHERE c.mode=? AND a.status='active'",mode).map(r=>({playerId:r.player_id,displayName:r.display_name,country:r.country,career:JSON.parse(r.value)}));return rankHallOfFame(rows,mode,viewerId,options);},leaderboard(mode){if(!['Duel','3 Players','4 Players'].includes(mode))throw Error('Unsupported ranking mode');const rows=all("SELECT c.player_id,c.value FROM stat_career c JOIN accounts a ON a.id=c.player_id WHERE c.mode=? AND a.status='active'",mode).map(r=>({playerId:r.player_id,...JSON.parse(r.value)}));const categories={'Siege Champion':['wins',-1],'The Underdog':['losses',-1],'Highest Win Ratio':['winRatio',-1],'Highest Loss Ratio':['lossRatio',-1],'Game Master':['full',-1],'Stormtrooper Award':['accuracy',1]};return Object.fromEntries(Object.entries(categories).map(([name,[key,direction]])=>[name,rows.map(r=>({playerId:r.playerId,value:key==='full'?r.reliability.Full:r[key]})).filter(r=>r.value!==null).sort((a,b)=>direction*(a.value-b.value)||a.playerId.localeCompare(b.playerId))]));},inspect(playerId){const modes=playerId?all('SELECT mode,value FROM stat_career WHERE player_id=?',playerId).map(r=>({mode:r.mode,...JSON.parse(r.value)})):[];return {modes,global:q('SELECT shots AS playerShots,cells AS cellsHit,plague AS plagueCells,scouted AS scoutedCells,unit_cells AS unitCellsHit FROM stat_global WHERE id=1'),scoreStatus:'MATCH_SCORE_V1',score:playerId?scoreReadModel(db,playerId):null};}};
}
