...
export function groupPresentation(project,target,onFrame){
 ...
 r.host={...live,state:stateView};refreshHost(r.host);state.projecting=true;try{for(let i=0;i<r.seats.length;i++){if(r.seats[i].controller!=='human')continue;if(live.state.match.knowledge[live.config.players[i].id].events.length<=state.positions[i])continue;const update=project(r,i,state.positions[i]),position=update.snapshot.eventPosition;if(position<=state.positions[i])continue;const j=target(r,i);if(j<0)continue;const view={...r.host,config:{...r.host.config,players:[r.host.config.players[i],r.host.config.players[j]]}},animation=animationCue(view,delta.filter(e=>!e.meta||[view.config.players[0].boardId,view.config.players[1].boardId].includes(e.meta.targetBoardId)).map(e=>e.meta?{...e,meta:{...e.meta,ownerId:view.config.players.some(p=>p.id===e.meta.ownerId)?e.meta.ownerId:view.config.players[1].id}}:e),state.groups[i]);if(animation?.kind==='archer'){const impact=delta.find(e=>e.kind==='impact'&&e.meta?.source==='archer');if(impact&&!view.config.players.some(p=>p.id===impact.meta.ownerId)){delete animation.origin;animation.originMode='off-board-right';}}// Only announce the observer's own executed Goblin reaction. No off-screen
