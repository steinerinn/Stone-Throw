import {demonPresentationRunes} from './demon-presentation.mjs';
...
export function groupPresentation(project,target,onFrame){
 ...
 function begin(r){r.presentation=r.seats.map(()=>[]);const state={cursor:r.host.events.length,positions:[],demonRunes:new Map(),groups:r.seats.map(()=>new Map()),projecting:true,pending:r.seats.map(()=>[]),offscreen:r.seats.map(()=>new Set())};states.set(r,state);state.positions=r.host.config.players.map(p=>r.host.state.match.knowledge[p.id].events.length);state.projecting=false;}
 ...
 r.host={...live,state:stateView};refreshHost(r.host);state.projecting=true;try{for(let i=0;i<r.seats.length;i++){if(r.seats[i].controller!=='human')continue;if(live.state.match.knowledge[live.config.players[i].id].events.length<=state.positions[i])continue;const update=project(r,i,state.positions[i]),position=update.snapshot.eventPosition;if(position<=state.positions[i])continue;const j=target(r,i);if(j<0)continue;const view={...r.host,config:{...r.host.config,players:[r.host.config.players[i],r.host.config.players[j]]}},animation=animationCue(view,delta.filter(e=>!e.meta||[view.config.players[0].boardId,view.config.players[1].boardId].includes(e.meta.targetBoardId)).map(e=>e.meta?{...e,meta:{...e.meta,ownerId:view.config.players.some(p=>p.id===e.meta.ownerId)?e.meta.ownerId:view.config.players[1].id}}:e),state.groups[i]);if(animation?.kind==='demon'){const impact=delta.find(e=>e.kind==='impact'&&e.meta?.source==='demon-blast'&&e.meta.targetBoardId===view.config.players[animation.side==='self'?0:1].boardId);animation.runes=demonPresentationRunes(live,r.memory,impact,state.demonRunes);}if(animation?.kind==='archer'){const impact=delta.find(e=>e.kind==='impact'&&e.meta?.source==='archer');if(impact&&!view.config.players.some(p=>p.id===impact.meta.ownerId)){delete animation.origin;animation.originMode='off-board-right';}}// Only announce the observer's own executed Goblin reaction. No off-screen
// cells, bomb count, target identity or hidden results enter the public payload.
const offscreen=delta.find(e=>e.kind==='impact'&&e.meta?.source==='goblin'&&e.meta.ownerId===r.host.config.players[i].id&&![view.config.players[0].boardId,view.config.players[1].boardId].includes(e.meta.targetBoardId));const noticeKey=offscreen&&offscreen.rootId+':'+offscreen.workId;const notice=noticeKey&&!state.offscreen[i].has(noticeKey);if(notice)state.offscreen[i].add(noticeKey);
const frame={...(notice?{offscreenAttack:'own-goblin'}:{}),snapshot:update.snapshot,events:impactSources(update.events.filter(e=>e.position>state.positions[i]),delta,r.host.config.players[i].boardId,r.host.config.players[j].boardId),...(animation?{animation}:{})};r.presentation[i].push(frame);const pending=state.pending[i],previous=pending.find(f=>f.animation)?.animation;if(previous&&animation&&(previous.group!==animation.group||previous.kind!==animation.kind))flush(r,i,state);state.pending[i].push(frame);if(!animation&&!previous)flush(r,i,state);state.positions[i]=position;}}finally{state.projecting=false;r.host=old;}}
