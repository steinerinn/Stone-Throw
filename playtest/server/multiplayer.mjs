...
export function createPvpService(roster,options){
 ...
 return {
  ...
  async lobby(action,body,binding){if(action==='make')body=entryBody(body);else checkEntryCode([...two.rooms.values(),...ring.rooms.values()],body);const code=String(body.code||'').trim().toUpperCase(),service=action==='make'?(body.seats===undefined||body.seats===2?two:ring):(ring.rooms.has(code)?ring:two);const result=await service.lobby(action,body,binding);owners.set(result.token,service);if(action==='make')stampEntry(service.rooms.get(result.update.lan.code),body,options?.now?.()??Date.now(),1+Math.max(0,...[...two.rooms.values(),...ring.rooms.values()].map(r=>r.entry?.order||0)));return result;},
