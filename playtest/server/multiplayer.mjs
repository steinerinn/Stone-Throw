...
export function createPvpService(roster,options){
 ...
 return {
  ...
  async route(token,...args){const service=owners.get(token);if(!service)throw Error('unknown-seat');const out=await service.route(token,...args);if(service!==ring||args[1]!=='rematch'||!out.lan?.complete)return out;const old=ring.rooms.get(out.lan.code),i=out.lan.self;const task=(rematchQueues.get(old)||Promise.resolve()).then(()=>moveRematch(old,i,args[0]));rematchQueues.set(old,task.catch(()=>{}));return task;},
