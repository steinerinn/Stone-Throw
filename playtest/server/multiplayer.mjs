...
export function createPvpService(roster,options){
 ...
 return {
  ...
  async route(token,...args){const service=owners.get(token);if(!service)throw Error('unknown-seat');if(args[1]==='rematch'){const room=[...service.rooms.values()].find(r=>r.seats.some(s=>s?.token===token)),seat=room?.seats.find(s=>s?.token===token),minimum=room?.entry?.minimumReliability||0;if(minimum){const rating=seat?.identity?.kind==='account'?options?.reliability?.(seat.identity.playerId):null;if(rating?.percent==null||rating.percent<minimum)throw Error('reliability-required-'+minimum);}}let out=await service.route(token,...args);if(service===two&&args[1]==='rematch'&&out.snapshot?.phase==='placement'){const room=two.rooms.get(out.lan.code);for(const seat of room.seats)if(seat?.controller==='human')Object.assign(seat,refreshed(seat));out=await service.route(token,args[0],'read');}if(service!==ring||args[1]!=='rematch'||!out.lan?.complete)return out;const old=ring.rooms.get(out.lan.code),i=out.lan.self;const task=(rematchQueues.get(old)||Promise.resolve()).then(()=>moveRematch(old,i,args[0]));rematchQueues.set(old,task.catch(()=>{}));return task;},
