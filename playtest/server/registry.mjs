...
`);
 ...
 run("UPDATE accounts SET avatar_id=? WHERE avatar_id IS NULL OR avatar_id='' OR avatar_id='registered-default'",DEFAULT_AVATAR);
 function refreshAvatar(id){const a=q('SELECT * FROM accounts WHERE id=?',id);if(!a)return a;
 ...
 async handle(action,b,{token,browser,ip}){
  ...
  if(action==='register'){
   ...
   const [username,k]=nameKey(b.username);password(b.password);if(b.password!==b.confirmPassword)fail('Passwords do not match.');const avatar=validateAvatar(b.avatarId===undefined?DEFAULT_AVATAR:b.avatarId),em=email(b.email),co=country(b.country),salt=randomBytes(16).toString('hex'),derived=(await key(b.password,salt)).toString('hex');
   return tx(()=>{if(q('SELECT id FROM accounts WHERE username_key=?',k))fail('Username is already taken.');if(q('SELECT id FROM accounts WHERE display_key=?',k))fail('Display name is already taken.');const id=randomUUID();run('INSERT INTO accounts(id,username,username_key,display_name,display_key,password_hash,salt,email,country,created_at,last_login,avatar_id) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)',id,username,k,username,k,derived,salt,em,co,now(),now(),avatar);return loginSession(id,token);});
  }
  if(action==='login'){
