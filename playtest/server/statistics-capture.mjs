import {finalReliability} from './reliability-outcome.mjs';
...
export function prepareStatistics(holder,h,epoch,{mode,build,now,participants,news=[],closed=false,gaveUp=false}){
 ...
 if(h.status==='complete'||closed||noHumans){d.endedAt=now();const outcome=h.state.match.outcome;if(h.status==='complete')d.finalResult={outcome:outcome.kind,placements:Object.fromEntries(h.config.players.map((p,i)=>[p.id,finalPlacements(h)[i]]))};
  for(const p of d.participants){if(p.kind==='ai')continue;p.outcome??=outcome.kind==='draw'?(h.events.findLast(r=>r.event.statistics?.eliminationBoundary?.revolt)?.event.statistics.eliminationBoundary.dead.includes(p.actor)===false?'Loss':'Draw'):outcome.kind==='win'&&outcome.winnerIds.includes(p.actor)?'Win':'Loss';p.reliability??='Full';if(d.classification.mode==='online'&&p.kind==='account'){const seat=participants[p.seat];p.reliabilityFacts={incidents:seat?.afkIncidents||0,disconnects:seat?.disconnects||0,departure:!!seat?.statDeparture||!!seat?.absence||p.reliability!=='Full',history:structuredClone(seat?.afkHistory||[])};p.reliabilityOutcome=finalReliability(p.reliabilityFacts);}const boundary=h.events.find(row=>row.event.statistics?.eliminationBoundary?.dead.includes(p.actor))?.event.statistics.eliminationBoundary;p.placement=boundary?.revoltScores?d.finalResult.placements[p.actor]:(p.outcome==='Win'||p.outcome==='Draw'?1:boundary?boundary.survivors+1:h.config.players.length);}
 }
 return d;
}
