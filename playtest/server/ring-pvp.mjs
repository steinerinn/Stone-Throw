...
export function createRingService(roster,{seed,onFailure}={}){
 const rooms=new Map(),tokens=new Map();const presentation=roomPresentation((r,i)=>read(r,i),target);
 ...
 const cmd=(r,c)=>{r.host=acceptCommand(r.host,{id:'ring-'+(++r.serial),...c},presentation.execution(r));};
 ...
 function aiStep(r,i){const h=r.host,owner=h.config.players[i],d=h.pendingRoot?.decisions.find(d=>d.status==='pending'),j=d&&d.boardId!==owner.boardId?h.config.players.findIndex(p=>p.boardId===d.boardId):h.config.players.findIndex(p=>p.id===normalTarget(h.state,owner.id).playerId);
  ...
  if(!d){const k=policy.target(h.state.plagues.some(p=>p.targetPlayerId===h.config.players[j].id));if(k)cmd(r,{kind:'shoot',actorId:owner.id,boardId:h.config.players[j].boardId,cell:parseKey(k)});else{h.turnStep='plague';h.status='running';pumpHost(h,100000,presentation.execution(r));}return;}
 ...
 function read(r,i,after=0){const h=r.host,p=h.config.players[i],j=target(r,i),visibleSelf=active(r,i),visibleTarget=j>=0&&active(r,j);const focus=j>=0?j:(i+1)%r.seats.length,v=view(r,i,focus),boards=new Set([...(visibleSelf?[p.boardId]:[]),...(visibleTarget?[h.config.players[focus].boardId]:[])]);
 ...
 async function dispatch(r,i,request){const current=await read(r,i),deny=error=>({...current,accepted:false,error,events:[],presentation:[]});if(!request||Object.keys(request).sort().join(',')!=='battle,contract,intent,revision')return deny('unsupported');if(request.contract!==current.snapshot.contract||request.battle!==current.snapshot.battle||request.revision!==r.revision)return deny('stale');
  ...
  const before={presentation:structuredClone(r.presentation),host:structuredClone(r.host),memory:structuredClone(r.memory),statistics:structuredClone(r.statistics),aiMemory:structuredClone(r.aiMemory),serial:r.serial,revision:r.revision,ready:[...r.ready]};presentation.begin(r);
