...
export function projectSeat(h,normalMemory,statistics,epoch,revision,unitHandles,choiceHandles,after=0){const self=h.config.players[0],opponent=h.config.players[1],side=p=>p===self.id?'self':'opponent',battle=()=>`battle-${epoch}`,ownHandle=uid=>unitHandles.get(uid),decision=()=>h.pendingRoot?.decisions.find(d=>d.status==='pending'&&d.actorId===self.id);
    ...
    const observer = projectObserver(h.state.match, self.id);
