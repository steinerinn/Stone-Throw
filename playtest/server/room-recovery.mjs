...
import {requiredInput} from './afk-policy.mjs';
...
export function restoreRooms(saved,rooms,tokens,transfer=false){
 ...
 for(const raw of saved){if(transfer||typeof raw.host!=='string')assertHost(raw.host);const r={...raw,host:transfer||typeof raw.host!=='string'?raw.host:deserializeHost(raw.host),queue:Promise.resolve(),epoch:randomInt(1,2**48),revision:raw.revision+1};
  ...
  // A server restart is not a new inactivity incident. Keep its votes/history,
  // exclude downtime, and wait for the recovered presentation to become usable.
  if(!transfer&&r.afk){const input=requiredInput(r);if(input){r.afk.key=input.key;r.afk.resumePending=true;}else r.afk=null;}
