...
export function entryBody(body){
 if(body.minimumReliability!==undefined&&(!Number.isInteger(body.minimumReliability)||body.minimumReliability<0||body.minimumReliability>100))throw Error('invalid-configuration');
...
export function stampEntry(room,body,now,order){room.entry={id:randomUUID(),visibility:body.visibility||'public',minimumReliability:body.minimumReliability||0,createdAt:now,order};}
...
export function listEntries(rooms,binding,now){
 ...
 return {roomId:id(r),minimumReliability:r.entry?.minimumReliability||0,...(visibility==='public'?{code:r.code}:{}),creator:r.seats[r.hostSeat??0].name,type:r.seats.length===2?'Duel':r.seats.length+'-player Group',seats:r.seats.length,occupied:r.seats.length-freeSlots,freeSlots,full:freeSlots===0,visibility,createdAt:r.entry?.createdAt??0,order:r.entry?.order??0};
