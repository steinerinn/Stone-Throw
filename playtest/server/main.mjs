...
export async function startServer({registryDir=registryDirectory(),port=3211,betaGameLog=false,development=false,seed,now=Date.now,lan=false,bind,stateDir,publicOrigin,secureCookies=false,playtestSnapshotOnly=false,statisticsInspector=false,hofPlaytest=false,recoverLocalSession,logger=event=>console.log(JSON.stringify(event))}={}){
 ...
 async function saveInner(){
  ...
  for(const [d,h]of captures)getRegistry().statistics.capture(d,h);
 }

 // Attach only the authenticated seat's persisted score, after save/finalization.
 function resultScore(body,req){
  const u=body?.snapshot?body:body?.update,s=u?.snapshot;if(!s||s.phase!=='finished'||s.online&&!s.online.complete)return body;
 ...
 try{if(store?.value){for(const e of store.value.local){sessions.set(e.token,restoreEntry(e));}pvp.restoreState(store.value.multiplayer);for(const r of pvp.rooms.values())for(const seat of r.seats)if(seat?.identity)seat.identity=guestAvatar(seat.identity);recovery='restored';await save();log('restore-success');}else if(store)recovery='new';}catch{store?.release();log('restore-failed',{reason:'checkpoint-unavailable'});throw Error('Checkpoint unavailable; no sessions served');}
...
let body;try{body=JSON.parse(req.intake||Buffer.alloc(0));if(!body||typeof body!=='object'||Array.isArray(body))throw Error();}catch{return reply(400,{error:'malformed-request'});}
   ...
   return await ordered(entry,async()=>{
    ...
    if(url.pathname==='/api/command'){
     ...
     const bindStory=update=>{if(slot.configuration.story&&update.accepted&&body.intent?.kind==='start'){if(slot.storyOwnerId===undefined)slot.storyOwnerId=onlineIdentity()?.playerId||null;if(slot.storyRunId===undefined)slot.storyRunId=slot.storyOwnerId?getRegistry().storyState(slot.storyOwnerId).runId:null;}return update;};
     ...
     const update=bindStory(await entry.session.dispatchWithProgress(body,async frame=>{send({type:'preview',frame});await new Promise(resolve=>setImmediate(resolve));}));
