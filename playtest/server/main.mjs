import {openRegistry,registryDirectory} from './registry.mjs';
import {metricsContext,measured,measuredAsync} from './beta-metrics.mjs';
...
export async function startServer({registryDir=registryDirectory(),port=3211,betaGameLog=false,development=false,seed,now=Date.now,lan=false,bind,stateDir,publicOrigin,secureCookies=false,playtestSnapshotOnly=false,logger=event=>console.log(JSON.stringify(event))}={}){
 ...
 const staticFiles=new Set([...assets,...narrationAssets,'styles-registry.css','client-v13/registry.js','styles-multiplayer.css','asset-manifest.json','client/story-presentation.js','client/loading-screen.js',...Array.from({length:23},(_,i)=>'styles-'+String(i).padStart(2,'0')+'.css'),...['placement','coordinates','footprints'].map(x=>'public-rules/'+x+'.js'),...['strength-public','strength-dataset','strength-renderer','game-log','combat-playback','legacy-animations','shell','presentation','action-instructions','resurrection-sparks','combat-feedback','battle-log','placement-feedback','castle-art-ready','story-browser','story-policy','story-tutorials','transport','bootstrap-production','lan','group-lan','rejoin',...(development?['bootstrap-development','node-development']:[])].map(x=>'client-v13/'+x+'.js'),'StoneThrow-v1.427-stage13-'+(development?'development':'production')+'.html']);
 const handle=async(req,res)=>metricsContext.run(betaGameLog&&req.headers['x-st-game-log']==='1'?{}:undefined,()=>handleInner(req,res));
 const handleInner=async(req,res)=>{const betaStart=performance.now(),betaWall=new Date().toISOString();const betaData=()=>({wall:betaWall,...(req.url==='/api/open'?{startupMetrics}:{}),requestMs:performance.now()-req.betaArrival,queueMs:betaStart-req.betaArrival,metrics:metricsContext.getStore()});
  const reply=async(status,body)=>{if(body?.snapshot||body?.update?.snapshot)metricsContext.getStore()&&(metricsContext.getStore()['public-result-boundary']={ms:performance.now()-betaStart,count:1});detachFormerPlayers();if(store&&!req.groupLight&&req.url.startsWith('/api/')&&!fatal){try{await save();}catch{fatal=true;log('checkpoint-write-failed');status=503;body={error:'server-recovery-unavailable'};}}const text=measured('response-serialization',()=>JSON.stringify(body));if(metricsContext.getStore())res.setHeader('X-St-Beta-Metrics',JSON.stringify(betaData()));res.writeHead(status,{'Content-Type':'application/json','Cache-Control':'no-store','X-Content-Type-Options':'nosniff'});res.end(text);};
  try{
...
let body;try{body=JSON.parse(req.intake||Buffer.alloc(0));if(!body||typeof body!=='object'||Array.isArray(body))throw Error();}catch{return reply(400,{error:'malformed-request'});}
   if(url.pathname.startsWith('/api/registry/')){
    ...
    try{const result=await getRegistry().handle(url.pathname.slice('/api/registry/'.length),body,{token:cookies.csAccount,browser,ip:req.socket.remoteAddress||'local'});
   ...
   function onlineIdentity(){
    ...
    if(!entry.guestIdentity){const pool=[{avatarId:'guest-goblin-fallback',avatarSrc:'assets/units/goblin.png'}];entry.guestIdentity={kind:'guest',playerId:null,guestId:'guest-'+randomBytes(16).toString('hex'),displayName:'Guest'+randomInt(100,1000),...pool[randomInt(pool.length)]};}
    return structuredClone(entry.guestIdentity);
   }
   if(url.pathname==='/api/pvp/identity'){if(!lan)return reply(403,{error:'lan-disabled'});if(!entry)return reply(404,{error:'unknown-session'});if(Object.keys(body).length)return reply(400,{error:'malformed-request'});return reply(200,{identity:onlineIdentity()});}
   detachFormerPlayers();
   ...
   if(url.pathname==='/api/command'&&entry?.mode==='multiplayer'&&pvp.isGroup(seatToken)&&req.headers.accept?.includes('application/x-ndjson')){res.writeHead(200,{'Content-Type':'application/x-ndjson','Cache-Control':'no-store','X-Content-Type-Options':'nosniff'});const send=v=>{if(!res.destroyed)res.write(measured('stream-serialization',()=>JSON.stringify(metricsContext.getStore()&&v.type==='result'?{...v,betaGameLog:betaData()}:v))+'\n');};try{const update=await pvp.stream(seatToken,token,body,u=>send({type:'progress',update:u}));if(metricsContext.getStore())metricsContext.getStore()['authoritative-commit-boundary']={ms:performance.now()-betaStart,count:1};await save();send({type:'result',update});}catch(e){send({type:'error',error:'group-command-failed'});}return res.end();}
 ...
 const server=http.createServer(async(req,res)=>{req.betaArrival=performance.now();try{if(req.method==='POST'){let bytes=0;const chunks=[];for await(const chunk of req){bytes+=chunk.length;if(bytes>65536){res.writeHead(413,{'Content-Type':'application/json'});res.end(JSON.stringify({error:'request-too-large'}));return;}chunks.push(chunk);}req.intake=Buffer.concat(chunks);}if(req.url.startsWith('/api/registry/')){req.groupLight=true;await handle(req,res);return;}const st=(req.headers.cookie||'').split(';').map(s=>s.trim()).find(s=>s.startsWith('st12seat='))?.slice(9);if((req.url==='/api/pvp/heartbeat'&&pvp.requiresExplicitReturn(st))||(req.url==='/api/read'&&pvp.isGroup(st)&&pvp.isBusy(st))){req.groupLight=true;await handle(req,res);return;}/* Read-only allowlisted assets must not wait behind an active combat transaction. */if(req.method==='GET'&&!req.url.startsWith('/api/')){await handle(req,res);return;}requestQueue=requestQueue.then(()=>handle(req,res)).catch(()=>{log('unexpected-server-error');if(!res.headersSent)res.writeHead(500,{'Content-Type':'application/json'});res.end(JSON.stringify({error:'server-error'}));});}catch{if(!res.destroyed)res.destroy();}});
 ...
 return {origin,lan,address,pvp,close:async()=>{stopping=true;clearInterval(policyTimer);const closed=new Promise(resolve=>server.close(resolve));await requestQueue;await pvp.close();try{if(!fatal)await save();}finally{await closed;store?.release();registry?.close();log('shutdown');}},
