 ...
 const handle=async(req,res)=>{
  ...
  try{
   ...
   if(url.pathname==='/api/mode'){
    ...
    if(body.mode==='multiplayer'){if(!lan||!seatToken)return reply(200,{available:false});try{const opened=await pvp.route(seatToken,token,'open',{});entry.mode='multiplayer';return reply(200,{available:true,...opened});}catch{return reply(200,{available:false});}}
   ...
   if(url.pathname==='/api/pvp/leave'){if(lan&&seatToken){try{await pvp.route(seatToken,token,'leave');}catch{}}if(entry)entry.mode=modeOf(entry.configuration);res.setHeader('Set-Cookie','st12seat='+cookieFlags+'; Max-Age=0');log('game-left');return reply(200,{left:true});}
   if(url.pathname.startsWith('/api/pvp/')||(seatToken&&(!entry?.mode||entry.mode==='multiplayer')&&['/api/open','/api/read','/api/command','/api/configure'].includes(url.pathname))){
    ...
    try{const action=url.pathname.split('/').at(-1);if(['make','join'].includes(action)){if(!entry)return reply(404,{error:'unknown-session'});const result=await pvp.lobby(action,body,token);res.setHeader('Set-Cookie',`st12seat=${result.token}${cookieFlags}`);delete result.token;entry.mode='multiplayer';log(action==='make'?'game-created':'seat-joined');return reply(200,result);}if(action==='configure')return reply(403,{error:'lan-configuration-locked'});const result=await pvp.route(seatToken,token,action,body);if(action==='open'&&entry)result.modes=Object.keys(slots(entry));return reply(200,result);}catch(e){return reply(400,{error:['unknown-seat','bad-game-code','game-full','invalid-name','already-seated','not-complete','invalid-configuration','unknown-session'].includes(e.message)?e.message:'invalid-request'});}
   }
   if(url.pathname==='/api/open'){
    ...
    return await ordered(entry,async()=>reply(200,{configuration:entry.configuration,reconnected,development,modes:Object.keys(slots(entry)),update:await entry.session.client.read()}));
   }
   if(!entry)return reply(404,{error:'unknown-session'});
