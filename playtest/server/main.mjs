...
let body;try{body=JSON.parse(req.intake||Buffer.alloc(0));if(!body||typeof body!=='object'||Array.isArray(body))throw Error();}catch{return reply(400,{error:'malformed-request'});}
   ...
   if(url.pathname==='/api/pvp/leave'){if(lan&&seatToken){try{await pvp.route(seatToken,token,'leave');}catch{}}if(entry){entry.mode='multiplayer-menu';entry.seatToken=null;}res.setHeader('Set-Cookie','st12seat='+cookieFlags+'; Max-Age=0');log('game-left');return reply(200,{left:true});}
