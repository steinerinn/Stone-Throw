...
export function hostFingerprint(host) { return stableJson(host); }
export function createPrivateReplay(initial) { assertMatchState(initial.state.match); const fingerprint = hostFingerprint(initial); return { contract: 'stone-throw-private-replay-v1', rulesVersion: initial.config.rulesVersion, initial: structuredClone(initial), initialFingerprint: fingerprint, steps: [], finalFingerprint: fingerprint }; }
function apply(host, action) { if (action.kind === 'policy-step')
    return autoStep(host); if (action.kind === 'command')
    return acceptCommand(host, action.command); throw Error('Unknown replay action'); }
...
export function replayPrivate(record) { if (record.contract !== 'stone-throw-private-replay-v1' || record.rulesVersion !== 'stone-throw-v1.427' || record.initial.config.rulesVersion !== record.rulesVersion)
    ...
    if (host.rng.cursor !== step.rngBefore)
        throw Error(prefix + 'RNG before');
    let next;
    try {
        next = apply(host, step.action);
    }
    catch (error) {
        throw Error(prefix + String(error));
    }
    if (next.rng.cursor !== step.rngAfter)
        throw Error(prefix + 'RNG after');
    if (stableJson(next.history.slice(host.history.length).map(r => r.command)) !== stableJson(step.accepted))
        throw Error(prefix + 'accepted command');
    if (hostFingerprint(next) !== step.fingerprint)
        throw Error(prefix + 'state/event/RNG fingerprint');
    assertMatchState(next.state.match);
    host = next;
} if (hostFingerprint(host) !== record.finalFingerprint)
    throw Error('Replay final fingerprint diverged'); return host; }
