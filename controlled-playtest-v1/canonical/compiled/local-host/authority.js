...
export function createLocalAuthority(config, checkpoint, development = false) {
    ...
    function capturePresentation(live) {
        ...
        const before = h;
        try {
            h = structuredClone(live);
            refreshHost(h);
            const s = snapshot(), e = events().filter(e => e.position > presentationCursor);
            presentation.push({ snapshot: s, events: e });
    ...
    const execution = () => ({ ...normalExecution(normalMemory), observePresentationStep: capturePresentation });
    function response(accepted = true, error = null, after = 0) { return structuredClone({ accepted, error, snapshot: snapshot(), events: accepted ? events().filter(e => e.position > after) : [], ...(accepted && presentation.length ? { presentation: presentation.filter(f => f.snapshot.eventPosition > after) } : {}) }); }
    ...
    const client = Object.freeze({ read: async (after = 0) => { await queue; if (!Number.isSafeInteger(after) || after < 0 || after > h.state.match.knowledge[self.id].events.length)
            throw Error('Invalid public event cursor'); return response(true, null, after); }, dispatch: (raw) => { const task = queue.then(() => { let request; try {
    ...
    return Object.freeze({ ...(dev ? { dev } : {}), client, serializePrivate: () => JSON.stringify({ contract: 'local-authority-checkpoint-v1', host: serializeHost(h), normalMemory, statistics, ...(presentation.length ? { presentation } : {}), revision, epoch, serial, unitHandles: [...unitHandles], choiceHandles: [...choiceHandles] }) });
}
