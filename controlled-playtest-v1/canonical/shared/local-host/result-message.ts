import type {HostState} from '../host/contracts.js';import type {NormalMemory} from './normal-policy.js';import {random} from '../combat/rng.js';
...
export function reserveResultMessage(host:HostState,memory:NormalMemory){
 if(memory.resultMessage!==undefined)return;
 const outcome=host.state.match.outcome;if(outcome.kind!=='win'&&outcome.kind!=='draw')throw Error('Result before terminal outcome');
