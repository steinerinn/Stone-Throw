...
export async function startServer({port=3211,development=false,seed,lan=false,bind,stateDir,publicOrigin,secureCookies=false,logger=event=>console.log(JSON.stringify(event))}={}){
 ...
 const staticFiles=new Set([...assets,'asset-manifest.json','client/story-presentation.js','client/loading-screen.js',...Array.from({length:23},(_,i)=>'styles-'+String(i).padStart(2,'0')+'.css'),...['placement','coordinates','footprints'].map(x=>'public-rules/'+x+'.js'),...['shell','presentation','action-instructions','resurrection-sparks','combat-feedback','story-browser','story-policy','story-tutorials','transport','bootstrap-production','lan',...(development?['bootstrap-development','node-development']:[])].map(x=>'client-v13/'+x+'.js'),'StoneThrow-v1.427-stage13-'+(development?'development':'production')+'.html']);
