...
export function createPvpService(roster,{seed}={}){
 ...
 async function project(r,i,after=0){const h=perspective(r,i),own=h.config.players[0].id;for(const u of h.state.match.units.filter(u=>u.ownerId===own))handle(r.handles[i],u.id,`own-${r.epoch}-${i}-`);for(const d of h.pendingRoot?.decisions.filter(d=>d.actorId===own)||[])handle(r.choices[i],d.id,`choice-${r.epoch}-${i}-`);
 ...
 async function read(r,i,after=0){return {...await project(r,i,after),lan:metadata(r,i)};}
 function drain(r){for(let n=0;n<1000&&r.host.status==='awaiting-turn';n++)r.host=acceptCommand(r.host,{id:'lan-'+(++r.serial),kind:'advance-turn'},normalExecution(r.memory));if(r.host.status==='running')throw Error('Unresolved host');sample(r);}
 ...
 async function dispatch(r,i,request){const current=await read(r,i);const deny=error=>({...current,accepted:false,error,events:[],presentation:[]});if(!request||Object.keys(request).sort().join(',')!=='battle,contract,intent,revision')return deny('unsupported');if(request.contract!==current.snapshot.contract||request.battle!==current.snapshot.battle||request.revision!==r.revision)return deny('stale');
  ...
  const before={host:structuredClone(r.host),memory:structuredClone(r.memory),statistics:structuredClone(r.statistics),serial:r.serial,ready:[...r.ready]};try{const actor=r.host.config.players[i];
