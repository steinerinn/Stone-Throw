...
export function openStore(directory,build,mode){
 ...
 try{
  ...
  return {value,release,write(value){const payload=JSON.stringify(value,(_,v)=>v instanceof Map?{$map:[...v]}:v),temp=file+'.tmp';const fd=fs.openSync(temp,'w',0o600);try{fs.writeFileSync(fd,JSON.stringify({format:checkpointId,build,mode,sha256:digest(payload),payload}));fs.fsyncSync(fd);}finally{fs.closeSync(fd);}fs.renameSync(temp,file);}};
